...
-- 自定义事件
Game.ON_GAME_START = "ON_GAME_START"  -- 数据准备完成，开始游戏事件。
Game.CHOOSE_THE_WORD = "CHOOSE_THE_WORD"  -- 选定文字事件。
Game.ON_PEOTRY_DATA_READY = "ON_PEOTRY_DATA_READY"  -- 诗词数据已经准备好。
Game.ON_LEVEL_COMPLETED = "ON_LEVEL_COMPLETED"  -- 胜利事件，当前诗词库已空时表示过关。
Game.ON_LEVEL_FAILURE = "ON_LEVEL_FAILURE" -- 胜挑战失败 。
-- 构造函数
...
function Game:onTouch(event, target)
...
-- 当手指离开时，处理：
function Game:onTouchEnded(event, target)
...
-- 判断是否挑战失败
function Game:checkFailed()
    if self.isFailed() then
        -- 失败事件 (此例中 PlayScene 有监听此事件 )
        self:dispatchEvent({name = Game.ON_LEVEL_FAILURE})
    end
    return HP_ <= 0
end
